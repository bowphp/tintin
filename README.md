...
Petit spécificité, le `#unless` quant à lui, il permet de faire une condition inverse du `#if`.
...
Pour le faire avec **tintin** on utilise la clause `loop`.
...
Vous avez peut-etre remarquer le `#stop` il permet de stoper l'execution boucle. Il y a aussi son conjoint le `#jump`, lui parcontre permet d'arrêter l'éxécution à son niveau et de lancer s'éxécution du prochain tour de la boucle.
